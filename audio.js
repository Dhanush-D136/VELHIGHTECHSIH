...
(function () {
  ...
  const startSongAudio = new Audio('/Music/Pachchai%20Nirame%20-%20Piano%20-%20Film%20-%20Alaipayuthey.mp3');
  startSongAudio.preload = 'auto';

  function playStartSong() {
    if (isMuted) return;
    try {
      startSongAudio.pause();
      startSongAudio.currentTime = 0;
      startSongAudio.play().catch(err => {
        console.warn('Start song audio playback blocked by browser policy:', err);
      });
    } catch (e) {}
  }

  function stopStartSong() {
    try {
      startSongAudio.pause();
      startSongAudio.currentTime = 0;
    } catch (e) {}
